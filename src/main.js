...
var CreatureGenPF = (function() {
	...
	var parseEssential = function(data) {
		...
		which has numbers after it OnList use a regex which is shorter*/
	 ...
	 * OnList modify getLineByName and getLineNumberByName to allow regex
	 */
	 ...
	 * OnList: add option for verbrocity during generaton.
	 */
	var parseSpecials = function(data) {
		...
		/* OnList add in nextLine support for cases where the special ability
	...
	var parseDefenses = function(data) {
		...
		var lineEndFnd = data.length;
		var termChars = [';'];
		var excluded = ['SR','DR','Immune','Resist','Weaknesses'];//OnList compensate for missing delimiters (SR on black dragons vs succubus)
		var rc = -1;
		var i = 0; 
	 ...
	 * preserve negatives and what not. OnList there's a Regex solution to this
	 ...
	 * surrounding roll equations with [[ ]] OnList, should be replaced with a
	 * single regex
	 * 
	 */
	 ...
	 * OnList strip trailing operands +1d6 and such
	 */
